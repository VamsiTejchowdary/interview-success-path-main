...
const IndexPage = () => {
  ...
  const handleLogin = async () => {
    ...
    setIsLoading(true);
    try {
      // Try email marketer login first (uses Supabase Auth)
      const { loginEmailMarketer } = await import("@/lib/emailMarketerAuth");
      const isEmailMarketer = await loginEmailMarketer(email, password);

      if (isEmailMarketer) {
        console.log('✅ Email marketer login successful, redirecting...');
        toast({ title: "Success", description: "Welcome, Email Marketer!" });
        
        // Use window.location for more reliable redirect
        setTimeout(() => {
          console.log('🔄 Navigating to dashboard...');
          window.location.href = '/email-marketer/dashboard';
        }, 1000);
        return;
      }

      // If not email marketer, try regular Supabase auth for other roles
      try {
      ...
      <div className="flex-1 flex pb-12 sm:pb-20 mt-8 sm:mt-12">
        ...
        <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
          ...
          <div className="absolute inset-0">
            ...
            <div
              className="absolute top-40 right-32 w-40 h-40 bg-gradient-to-br from-blue-400/25 to-indigo-400/25 rounded-full blur-xl animate-pulse"
              style={{ animationDelay: "1s" }}
            ></div>
            <div
              className="absolute bottom-32 left-1/3 w-36 h-36 bg-gradient-to-br from-indigo-400/30 to-purple-400/30 rounded-full blur-xl animate-pulse"
            ...
            <div className="space-y-6 mb-12">
              {[
                ...
                },
                {
                  ...
                  desc: "Monitor your applications",
                  delay: "0.2s",
                },
                {
                  icon: Users,
